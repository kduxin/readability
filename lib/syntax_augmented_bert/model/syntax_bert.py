...
from .crf import CRF
from .loss import SequenceCriteriaCRF
...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, z, attention_mask, output_hidden_states=False):
        all_hidden_states = ()
        all_attentions = ()
    ...
    def forward(self, input_ids, token_type_ids=None,
        ...
        if self.config.model_type == "late_fusion":
            sequence_output = self.syntax_encoder(sequence_output,
                                                  ...
                                                  seq_len)
            if output_hidden_states:
                hidden_states = hidden_states + (sequence_output,)
