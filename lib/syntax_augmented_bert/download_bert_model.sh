...
S3_BASEURL='https://huggingface.co/'
...
do
    ...
    wget ${S3_BASEURL}${model}/resolve/main/${VOCAB_SUFFIX} -O ${MODEL_CACHE}'vocab.txt'
    wget ${S3_BASEURL}${model}/resolve/main/${MODEL_SUFFIX} -O ${MODEL_CACHE}'pytorch_model.bin'
    wget ${S3_BASEURL}${model}/resolve/main/${CONFIG_SUFFIX} -O ${MODEL_CACHE}'config.json'
done
...
do
    ...
    wget ${S3_BASEURL}${model}/resolve/main/${VOCAB_SUFFIX} -O ${MODEL_CACHE}'vocab.json'
    wget ${S3_BASEURL}${model}/resolve/main/${MERGES_SUFFIX} -O ${MODEL_CACHE}'merges.txt'
    wget ${S3_BASEURL}${model}/resolve/main/${MODEL_SUFFIX} -O ${MODEL_CACHE}'pytorch_model.bin'
    wget ${S3_BASEURL}${model}/resolve/main/${CONFIG_SUFFIX} -O ${MODEL_CACHE}'config.json'
done
